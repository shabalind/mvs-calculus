import java.lang.System.nanoTime
import scala.collection.immutable.Vector
object Gen extends App {
  case class s0 (
    p0: Vector[Vector[Double]],
    p1: Vector[Vector[Double]],
    p2: Vector[Vector[Double]],
    p3: Vector[Vector[Double]]
  )
  case class s1 (
    p0: s0,
    p1: s0
  )
  case class s2 (
    p0: Vector[Vector[s0]],
    p1: s1
  )
  case class s4 (
    p0: Vector[Vector[s1]],
    p1: s2
  )
  case class s6 (
    p0: s0,
    p1: s1
  )
  @noinline
  def f0(v0: s6, v1: Vector[Double], v2: s2, v3: Vector[s4], v4: Vector[Vector[Double]], v5: s1, v6: Double): Double = {
    val v13: Double = v6 / v6
    val v15: Double = v1(0)
    var v14: Vector[Double] = v1
    v14 = v14.updated(0, v6)
    val v19: Vector[Double] = v4(1)
    v14 = v14.updated(0, v13)
    val v28: Double = v19(0)
    var v52: Double = v6
    v14 = v14.updated(0, v15)
    val v41: Double = v14(0)
    val v241: Double = v1(0)
    v14 = v14.updated(0, v41)
    val v160: Double = v28 - v241
    val v173: Double = v160 + v41
    v14 = v14.updated(0, v52)
    v173
  }
  def benchmark(): Unit = {
    val v0: s6 = s6(s0(Vector(Vector(0.0), Vector(1.0)), Vector(Vector(2.0)), Vector(Vector(3.0), Vector(4.0)), Vector(Vector(5.0))), s1(s0(Vector(Vector(6.0), Vector(7.0)), Vector(Vector(8.0)), Vector(Vector(9.0), Vector(10.0)), Vector(Vector(11.0))), s0(Vector(Vector(12.0), Vector(13.0)), Vector(Vector(14.0)), Vector(Vector(15.0), Vector(16.0)), Vector(Vector(17.0)))))
    val v1: Vector[Double] = Vector(18.0)
    val v2: s2 = s2(Vector(Vector(s0(Vector(Vector(19.0), Vector(20.0)), Vector(Vector(21.0)), Vector(Vector(22.0), Vector(23.0)), Vector(Vector(24.0)))), Vector(s0(Vector(Vector(25.0), Vector(26.0)), Vector(Vector(27.0)), Vector(Vector(28.0), Vector(29.0)), Vector(Vector(30.0)))), Vector(s0(Vector(Vector(31.0), Vector(32.0)), Vector(Vector(33.0)), Vector(Vector(34.0), Vector(35.0)), Vector(Vector(36.0))))), s1(s0(Vector(Vector(37.0), Vector(38.0)), Vector(Vector(39.0)), Vector(Vector(40.0), Vector(41.0)), Vector(Vector(42.0))), s0(Vector(Vector(43.0), Vector(44.0)), Vector(Vector(45.0)), Vector(Vector(46.0), Vector(47.0)), Vector(Vector(48.0)))))
    val v3: Vector[s4] = Vector(s4(Vector(Vector(s1(s0(Vector(Vector(49.0), Vector(50.0)), Vector(Vector(51.0)), Vector(Vector(52.0), Vector(53.0)), Vector(Vector(54.0))), s0(Vector(Vector(55.0), Vector(56.0)), Vector(Vector(57.0)), Vector(Vector(58.0), Vector(59.0)), Vector(Vector(60.0))))), Vector(s1(s0(Vector(Vector(61.0), Vector(62.0)), Vector(Vector(63.0)), Vector(Vector(64.0), Vector(65.0)), Vector(Vector(66.0))), s0(Vector(Vector(67.0), Vector(68.0)), Vector(Vector(69.0)), Vector(Vector(70.0), Vector(71.0)), Vector(Vector(72.0))))), Vector(s1(s0(Vector(Vector(73.0), Vector(74.0)), Vector(Vector(75.0)), Vector(Vector(76.0), Vector(77.0)), Vector(Vector(78.0))), s0(Vector(Vector(79.0), Vector(80.0)), Vector(Vector(81.0)), Vector(Vector(82.0), Vector(83.0)), Vector(Vector(84.0)))))), s2(Vector(Vector(s0(Vector(Vector(85.0), Vector(86.0)), Vector(Vector(87.0)), Vector(Vector(88.0), Vector(89.0)), Vector(Vector(90.0)))), Vector(s0(Vector(Vector(91.0), Vector(92.0)), Vector(Vector(93.0)), Vector(Vector(94.0), Vector(95.0)), Vector(Vector(96.0)))), Vector(s0(Vector(Vector(97.0), Vector(98.0)), Vector(Vector(99.0)), Vector(Vector(100.0), Vector(101.0)), Vector(Vector(102.0))))), s1(s0(Vector(Vector(103.0), Vector(104.0)), Vector(Vector(105.0)), Vector(Vector(106.0), Vector(107.0)), Vector(Vector(108.0))), s0(Vector(Vector(109.0), Vector(110.0)), Vector(Vector(111.0)), Vector(Vector(112.0), Vector(113.0)), Vector(Vector(114.0)))))))
    val v4: Vector[Vector[Double]] = Vector(Vector(115.0), Vector(116.0))
    val v5: s1 = s1(s0(Vector(Vector(117.0), Vector(118.0)), Vector(Vector(119.0)), Vector(Vector(120.0), Vector(121.0)), Vector(Vector(122.0))), s0(Vector(Vector(123.0), Vector(124.0)), Vector(Vector(125.0)), Vector(Vector(126.0), Vector(127.0)), Vector(Vector(128.0))))
    val v6: Double = 129.0
    val start = nanoTime()
    var result: Double = 129.0
    (1 to 1000).foreach { _ =>
      result = f0(v0, v1, v2, v3, v4, v5, v6)
    }
    val end = nanoTime()
    println(result)
    println(end - start)
  }
  benchmark()
}